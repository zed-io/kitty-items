  ...
  setupAccount = () => {
    ...
    const transaction = fs
      .readFileSync(
        path.join(
          __dirname,
          `../../../kitty-items-cadence/cadence/nftStorefront/transactions/setup_account.cdc`
        ),
        "utf8"
      )
      .replace(/0xNFTSTOREFRONT/gi, `0x${this.storefrontAddress}`);
  ...
  getItem = (account: string, itemId: number) => {
    const script = fs
      .readFileSync(
        path.join(
          __dirname,
          `../../../kitty-items-cadence/cadence/nftStorefront/scripts/read_sale_offer_details.cdc`
        ),
        "utf8"
      )
      .replace(/0xNFTSTOREFRONT/gi, `0x${this.storefrontAddress}`);
  ...
  getItems = (account: string) => {
    const script = fs
      .readFileSync(
        path.join(
          __dirname,
          `../../../kitty-items-cadence/cadence/nftStorefront/scripts/read_storefront_ids.cdc`
        ),
        "utf8"
      )
      .replace(/0xNFTSTOREFRONT/gi, `0x${this.storefrontAddress}`);
  ...
  buy = (account: string, itemId: number) => {
    ...
    const transaction = fs
      .readFileSync(
        path.join(
          __dirname,
          `../../../kitty-items-cadence/cadence/nftStorefront/transactions/buy_market_item.cdc`
        ),
        "utf8"
      )
  ...
  sell = (itemId: number, price: number) => {
    ...
    const transaction = fs
      .readFileSync(
        path.join(
          __dirname,
          `../../../kitty-items-cadence/cadence/nftStorefront/transactions/sell_market_item.cdc`
        ),
        "utf8"
      )
      .replace(/0xFUNGIBLETOKENADDRESS/gi, `0x${this.fungibleTokenAddress}`)
